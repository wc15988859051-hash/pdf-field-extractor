...
try:
    ...
    print("请执行: pip install openpyxl==3.1.5")
    sys.exit(1)
...
# 前缀清理规则
PREFIX_CLEANUP = ['PO#', 'style code#', 'color code#', 'GBP', '¥', '$']
...
def merge_cells_for_group(ws, start_row: int) -> None:
    """
    为一组数据合并单元格
...
def fill_data_to_sheet(ws, data: List[Dict[str, Any]], start_idx: int = 0) -> None:
    """
    将数据填充到工作表，并合并单元格
    ...
    for idx, item in enumerate(data):
        ...
        # 清理已存在的前缀（避免重复添加）
        po = clean_prefix(po)
        style_code = clean_prefix(style_code)
        color_code = clean_prefix(color_code)
        sell = clean_prefix(sell)
        total = clean_prefix(total)
...
def main():
    """主函数"""
    if len(sys.argv) < 4:
        print("使用方法: python excel-exporter.py <json_data_path> <template_path> <excel_output_path>")
        print("")
        print("参数说明:")
        ...
        print("")
        print("示例:")
        print("  python excel-exporter.py ./extracted_data.json ./template.xlsx ./output.xlsx")
        print("")
        print("字段映射关系:")
    ...
    try:
        ...
        # 更新 Excel 文件（增量更新）
        update_excel(data, template_path, excel_path)
